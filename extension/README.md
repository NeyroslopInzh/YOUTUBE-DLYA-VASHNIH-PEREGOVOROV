...
Companion режет клип во **временную системную** папку (`%LOCALAPPDATA%\\Temp\\yvp-companion` / `/tmp/yvp-companion`), **не в папку проекта**. Расширение отдаёт файл в **менеджер загрузок Chrome** — путь берётся из `chrome://settings/downloads` (у тебя `D:\\Downloads1` или диалог, если включён «Запрашивать папку»).
